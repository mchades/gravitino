...
import static org.apache.gravitino.integration.test.util.ProcessData.TypesOfData.OUTPUT;
...
public class HiveContainer extends BaseContainer {
  ...
  public void copyFileFromContainer(String containerPath, String destinationPath) {
    if (activeCI) {
      CommandExecutor.executeCommandLocalHost(
          String.format("docker cp %s:%s %s", containerName, containerPath, destinationPath),
          false,
          OUTPUT);
    } else {
  ...
  public void copyFileToContainer(MountableFile mountableFile, String containerPath) {
    if (activeCI) {
      ...
      CommandExecutor.executeCommandLocalHost(
          String.format("docker cp %s %s:%s", file.getAbsolutePath(), containerName, containerPath),
          false,
          OUTPUT);
    } else {
